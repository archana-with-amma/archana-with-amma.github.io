...
app.controller("ResultsCtrl", [
  "$scope", "$localStorage", '$stateParams', '$http', 'dateFilter', "$window", "History", function($scope, storage, $stateParams, $http, dateFilter, $window, history) {}, $scope.id = $stateParams.itemId, state = storage[$scope.id], $scope.buttonColor = "btn-primary", $scope.incorrect = state.incorrect.map(function(id) {
    return {
      next: state.listToLearn[id + 1],
      previous: state.listToLearn[id]
    };
  }), $scope.quizletText = "Export to Quizlet", state.restart = true, now = new Date(), history.add(state.incorrect), $scope.exportQuizlet = function() {
